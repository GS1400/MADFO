...
%rng_octave(x) establishes the seed and reports seed and state for 
% the random number generator of rand, rande, randg, randn and randp
%x must be:
%- a numeric value that sets the seed value of the rng
%- 'shuffle': it seeds rng based on current time
%- 'default': it sets the seed = 0 (actually 5489 for Matlab 
% compatibility)
% if x is empty or missing, rng returns the seed of the random number
% generator
%
% Reference:https://viewer.mathworks.com

  errmsg = ['only a single numeric or limited string argument',...
           ' is supported, type help rng_octave for details'];
  if nargin == 0
    x = [];
  elseif nargin > 1
    error(errmsg);
  ...
  if isempty(x)
    ...
    if isequal(rand_state,rande_state,randg_state,...
                                         randn_state,randp_state)
  ...
  out.Type = 'twister';
  % states in Octave and Matlab are different, this fixes the difference
  % for rand
  % current limitations:
  % - unfortunately it does not seem to work for matching randn
  % - in Matlab, the same rng applies to rand and randn, whereas in 
  %   Octave each one evolves in its way. This means that in Matlab rand,
  %   randn and randn, rand give different series, whereas in Octave they 
  %   are the same
  function state = twister_state(seed)
    if nargin < 1 || seed == 0
      seed = 5489; % for Matlab compatibility, 0 and 'default' actually
      % corresponds to seed 5489 in Matlab
    end
    ...
    for N = 1:623
        ...
        % has to be uint64, otherwise in 4th iteration hit maximum
        % of uint32!
        uint64(1812433253)*uint64(bitxor(state(N),...
            bitshift(state(N),-30)))+N; 
        % untempered numbers
        state(N+1) = uint32(bitand(ans,uint64(intmax('uint32')))); 
    end
    % Matlab displays 624 here, but for compatibility, 
    % this has to be set to 1
    state(end) = 1; 
  end
end
